...
source("R/plot_simulated_data.R")
source("R/draw_init_values.R")
source("R/run_stan_models.R")
# Models

m1_euler = stan_model("models/model1_Euler_V1.stan")
m1_RK = stan_model("models/model1_RK_V1.stan")
model1_EU2 = ("models/model1_Euler_V2.stan")
...
**A3: why would we want to estimate these parameters?**
...
**Q5: Come up with a prior for $\beta$ which supports $\R_0$ ranging from 1 - ~12, given $\rho$ values ranging from 0-1.**
...
The data block includes the number of data points we have, as well as the number of time steps to run the model over. These two values aren't the same as we don't expect to have observed the transmission of Omega from the very first day of its emergence. To account for this, we seed our model ($I_0$) a month before we observe data. The time point that we seed Omicron is provided in the data block. 
...
Next, we simulate our transmission data by solving a set of ODE's using the function *simulate_data*. The model equations are defined in *models/model1_deSolve*. 

The function requires that we provide the % of immunity in the population, the 
initial number of infected individuals, the reporting probability, the transmission rate and the time steps at which to solve the model. 

We can additionally set the population and recovery and progression rates, if we don't want to use the predefined values. 

The function will return a data frame of solutions to the derivatives of all compartments at each time step. It will also print the plot of each compartment across time, so we can check everything is behaving as expected! 
...
```{r}
simulated_data = plot_simulated_data(
...
stan_fit_EU = run_stan_models(
  list_data =
    list(
      ...
      y = inc_sim$rep_inc,      # incidence data 
      n_data = dim(inc_sim$rep_inc)[1], # no. data
...
We obtained no immediate warnings about our model, great news! To be on the safe side, lets check with some model diagnostics. *diagnose_stan_fit* takes our fitted stan results and returns the number of divergent transmission, diagnostics plots and a table of summary statistics. 

```{r}
diagnose_stan_fit(stan_fit_EU, params = c("R0", "beta", "rho", "I0"))
```

The first output tells us that that were no divergent transitions. Check out what divergent transitions are and why they can *never* be ignored [here](https://mc-stan.org/docs/2_29/reference-manual/divergent-transitions.html).

The next output shows markov chain trace plots to check for model convergence, as well as univariate and bivariate marginal posterior distributions. Note, the strong correlation between $\beta$ and $I0$.
...
stan_fit_RK = run_stan_models(
  list_data =
    list(
      ...
      n_recov = n_recov,        # recovered population 
      y = inc_sim$rep_inc,      # incidence data 
      n_data = dimlength(inc_sim$rep_inc)[1], # no. data 
      sigma = sigma,            # progression rate  
      gamma = gamma,            # recovery rate 
      time_seed_omicron = seed_omicron, # time point to seed omicron 
...
```{r}
diagnose_stan_fit(stan_fit_RK, params = c("R0", "beta", "rho", "I0"))
```
...
**A7: the Euler method is XX faster**
...
**A8: Although both methods recover the parameter estimate for $\rho$, the RK method is more accurate than the Euler method at recovering the values for $\beta$ and $I_0$. The bivariate marginal posterior distributions plot showed strong correlation between $\beta$ and $I0$, indicating the uncertainty in these parameter's posteriors are correlated given the data. Therefore, the model is not able to accurately recover their true values.** 



**Q9: Reduce the time step at which *model1_Euler_V1.stan* solves the ODEs to improve the accuracy of the fit** 

**A9: *model1_Euler_V2.stan* modifies the orignal model to allow us to estiamte the ODE solutions at a scaled time step, which is smaller than 1 day. We now just need to provide an additional data variable, the amount to scale by.**
...
**Let's check the diagnostics now and see if we are able to recover the paramater values. **

```{r}
diagnose_stan_fit(stan_fit_EU2, params = c("R0", "beta", "rho", "I0"))
```
