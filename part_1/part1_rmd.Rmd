...
stan_fit_EU = run_stan_models(
  list_data =
    list(
      ...
      time_seed_omicron = which(all_dates == seed_omicron) # index to seed omicron
    ), 
  model = m1_EU 
)
```
...
stan_fit_RK = run_stan_models(
  list_data =
    list(
      ...
      sigma = sigma,                    # progression rate  
      gamma = gamma,                    # recovery rate 
      time_seed_omicron = which(all_dates == seed_omicron), # index seed omicron 
...
**Q10: fit both model variations to the real data *raw_data*. The *raw_data* is provided for all districts of South Africa between 05-03-2020 and 08-06-2022. The column GP refers to Gauteng. The data provided is cumulative so you will need to convert to daily incidence data.**
...
```{r}
obs_inc = tidy_obs_inc(raw_data = raw_data,
              date_seed = seed_omicron,
              end_date = end_date
              )

plot(obs_inc$date,obs_inc$cases)
```
 
 **Fit the EU model to the observed data**
